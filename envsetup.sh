...
function cmremote()
{
    ...
    CMUSER=`git config --get review.review.cas-online.nl:8181.username`
    if [ -z "$CMUSER" ]
    then
        git remote add cmremote ssh://review.cas-online.nl:29418/$GERRIT_REMOTE
    else
        git remote add cmremote ssh://$CMUSER@review.cas-online.nl:29418/$GERRIT_REMOTE
    fi
...
function cmgerrit() {
    ...
    local user=`git config --get review.review.cas-online.nl:8181.username`

...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:androidarmv6+in:name+fork:true" % device)
    add_auth(githubreq)
    ...
    try:
        ...
        sys.exit()
    if (numresults == 0):
        print("Could not find device %s on github.com/androidarmv6" % device)
        sys.exit()
